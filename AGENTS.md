...
- `internal/agent`：中立 mode 契约、公共 prompt 模板变量与 system-init spec；`internal/agent/builtin` 是 CODER/KBASE 的唯一静态分派点。
- `internal/agent/coder`：CODER profile、prompt、planning、ACP/workspace 策略与创建默认策略。
- `internal/agent/kbase`：KBASE profile、配置/默认值、prompt、索引、存储、watcher、HTTP 业务错误与五个工具 handler。
...
- `internal/catalog`：agent / team / skill / tool 目录装载与定义解析；Team 只提供成员与会话范围，并以原子快照参与 run 准入。
...
这里没有类继承：`internal/agent` 是中立契约层，`internal/agent/coder` 与 `internal/agent/kbase` 是该契约下的具体 mode 实现，`internal/agent/builtin` 只负责静态分派。Team 与这些 mode 平行，是引用多个 `AgentDefinition` 的 catalog 聚合与会话范围；Team 不派生 Agent，也不存在 `TEAM` mode。
...
│   └── agent/                   # 中立 mode 契约及 CODER/KBASE 特有实现
├── scripts/                     # 审计和辅助脚本
├── Dockerfile
├── Makefile
├── compose.yml
...
- 通用运行时配置事实源以 `internal/config/config.go` 和 `configs/*.example.yml` 为准；CODER/KBASE 的 profile、默认值和业务规则分别以 `internal/agent/coder`、`internal/agent/kbase` 为准，文档只解释和引用。
...
- Team 不是 agent mode；不得新增 `TEAM` mode、Team system prompt 或 `internal/agent/team`。mode 公共机制进入 `internal/agent`，特有规则进入对应 mode 包。
...
- chat 创建后 `teamId` 固定；Team 成员与默认 agent 在 run 开始时解析为快照，运行中 catalog 热重载不改变该 run 的准入范围。

## 特色功能文档索引

- [智能体配置说明](docs/智能体配置说明.md)：agent / team / skill 定义、CODER、KBASE、Team 边界、prompt files、memoryConfig、runtimeConfig。
