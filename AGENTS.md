...
- 已具备固定 Schema 的 `platform_control` system control plane：operation profile 与 Agent `runtimeConfig.runEnv` 分层授权，当前 root run 动态环境使用并发 State、AES-GCM checkpoint、operation-aware barrier/HITL 和 Host/Container command snapshot；不修改 Platform 进程环境。
...
- `internal/platformcontrol` 与 `internal/runenv`：统一 system control operation registry/profile handler，以及当前 run policy、并发 State、HMAC 幂等和 AES-GCM checkpoint。
...
- `platform_control` 对所有 Agent 使用同一固定 Schema，但仍需 Agent 显式挂载 Tool；Skill 不授予 operation。profile 由 `configs/tools.yml` 决定，动态 key 由 Agent `runtimeConfig.runEnv` 决定。旧 `platform_config` 配置硬失败。
...
- [Platform控制工具设计](docs/Platform控制工具设计.md)：`platform_control` operation、profile、run-local 环境快照、并发、脱敏、恢复与执行通道边界。
