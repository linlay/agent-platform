...
- `AP_AGENT_CONFIG_HOME`、`AP_WORKSPACE_DIR`、`AP_CHAT_DIR` 与 `AP_ACCESS_TOKEN` 是 Platform 保留变量，agent、skill 和调用级 env 均不得覆盖。前三者在 host bash/tool、Container Hub 与 Workspace Terminal 使用各自 canonical 路径；`AP_ACCESS_TOKEN` 仅在普通 Agent Host Bash 创建前从 `--identity-file` 指定的单行文件即时读取并注入，不进入 Terminal、Container、Proxy、ACP、MCP、LSP 或 sidecar。文件缺失、不可读、为空或非法时省略该变量，不缓存也不中断 Bash。
