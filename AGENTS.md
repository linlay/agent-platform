...
- steer 支持 `selection.meta.text` 纯文本选区（不要求视觉模型），以及通过 `/api/upload` 上传后以 `references` 注入当前 native Agent/Team 协调器的图片，准入冻结图片输入，公开事件仅带引用，JSONL 保存消息快照并进入续聊/压缩；文字仍必填，远端含图路径尚不支持。
