...
- 已具备 automation、`agent_invoke` 子智能体调度、`agent_run` 独立 Agent/Team 根 run 启动与控制、带隐藏协调器的 orchestrated Team、基于官方 Go SDK v1.6.1 的 MCP streamable HTTP/stdio session client 与 tool sync、WebSocket 控制面等能力骨架；MCP 唯一稳定协议版本为 `2025-11-25`。
...
- `internal/agentrun`：显式挂载的 `agent_run` named handler、调用方/subject 所有权、父 run/tool ID 幂等与禁止链式调用；实际 query admission、detached executor 和 Proxy 控制复用 `internal/server` facade。
...
│   ├── agentrun/                # agent_run handler、所有权与幂等
...
- `agent_run` 只允许显式配置的普通主 Agent 根 run 使用，按精确 catalog `agentKey/teamId` 启动独立根 run；不设目标白名单、深度/并发配置或 maxActiveRuns。控制动作只接受同一调用 Agent 与 subject 创建的 run，目标 run 禁止再次调用 `agent_run`。
...
- [子智能体调度](docs/子智能体调度.md)：`agent_invoke`、TEAM 隐藏调度与 `agent_run` 独立根 run 控制。
