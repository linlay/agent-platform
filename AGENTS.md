...
- `control.db`：schema v4 控制面，记录 generation、文件状态、file operation、增量 refresh 指标和 index run；不保存 chunk、FTS 或 embedding。控制面 schema 版本与 Lance index schema 版本独立，控制面迁移不会触发 generation 重建。
