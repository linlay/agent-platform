...
- `internal/agent/kbase`：KBASE profile、配置/默认值、prompt、索引、watcher、SQLite/Lance retrieval adapter、control store、generation/迁移/恢复、sidecar supervisor、HTTP 业务错误与五个工具 handler。
...
- `kbase.db`：保留的 schema v2 SQLite 检索库，用于 `auto` 迁移前服务和 `sqlite` 紧急回滚。
- `control.db`：schema v3 控制面，记录 generation、migration、file operation 和 index run。
- `generations/<generationId>/lance/`：LanceDB chunks table 及索引；`validation.json` 保存迁移检索验证结果。
- `migrations/<migrationId>.snapshot.db`：通过 SQLite online backup API 产生的一致性临时快照，成功激活后删除。
...
- KBASE 对外 tool/REST/`source.publish` 契约必须同时在 SQLite 与 LanceDB 路径上回归；只有 `indexHash` 变化可触发新 generation，`queryHash` 中的 topK/RRF/权重/候选池调整不得引发全量重建。
...
- KBASE Lance sidecar 只监听 loopback，由 Go 生成一次性 Bearer token 并监督生命周期。`auto` 模式在无已验证 generation 时继续由 SQLite 服务，sidecar/迁移失败不切流；已激活 Lance 后 sidecar 故障会显式返回 unavailable，不会回退到冻结 legacy。这些故障不影响非 KBASE 模块启动。
