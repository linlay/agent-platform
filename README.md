...
本仓库是 `agent-platform` 的 Go 版运行时实现，当前以 Java runtime 的 `.env` / `application.yml` 契约为事实源，支持目录驱动的 agents / teams / skills catalog、带隐藏协调器的 orchestrated Team、独立根 run 控制工具 `agent_run`、JWT 鉴权、resource ticket、chat 文件落盘、memory learn、Container Hub sandbox、LanceDB 本地混合检索 KBASE，以及最小 OpenAI 协议模型与 backend tool loop。
...
普通主 Agent 还可显式挂载 `agent_run`，用 `query/status/submit/steer/interrupt` 启动和控制标准独立 Agent/Team 根 run。它与 `agent_invoke` 不同：不复用父 `chatId/runId`，`query` 在目标 run 注册后立即返回，父 run 中断不取消目标；后续控制只允许同一调用 Agent 与 subject 操作自己通过该工具创建的 run。目标不使用白名单或 `contextConfig.agents`，精确 catalog 名称存在即可调用；首版禁止目标 run 链式调用 `agent_run`。完整契约见 [子智能体调度](./docs/子智能体调度.md)。
...
`context tags` 不是全局默认集合，而是每个 agent 从 `contextConfig.tags` 读取。当前支持/归一化后的标签有 `system`、`session`、`owner`、`agents`。`agents` 只表示向 prompt 注入 agent 摘要上下文，不授予 `agent_invoke`、`agent_run`、channel 或 catalog 权限，也不是 `agent_run` 目标白名单；`contextConfig.agents` 缺省时表示全部，也可用 YAML list 或逗号字符串指定部分 agent key。
...
- [自动化](./docs/自动化.md)
- [智能体调度（含 `agent_invoke`、TEAM 隐藏调度与独立 `agent_run`）](./docs/子智能体调度.md)
