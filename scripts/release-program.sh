...
require_file "$PROGRAM_RELEASE_ASSETS_DIR/windows/access-policy.example.yml"
require_file "$PROGRAM_RELEASE_ASSETS_DIR/windows/bash.example.yml"
require_file "$PROGRAM_RELEASE_ASSETS_DIR/windows/file-tools.example.yml"
...
build_program_bundle() {
  ...
  if [[ "$target_os" == "windows" ]]; then
    cp "$PROGRAM_RELEASE_ASSETS_DIR/windows/access-policy.example.yml" "$bundle_root/configs/access-policy.example.yml"
    cp "$PROGRAM_RELEASE_ASSETS_DIR/windows/bash.example.yml" "$bundle_root/configs/bash.example.yml"
    cp "$PROGRAM_RELEASE_ASSETS_DIR/windows/file-tools.example.yml" "$bundle_root/configs/file-tools.example.yml"
  fi
