...
func reviewASTCommand(command string, cmd bashast.SimpleCommand) ReviewResult {
	...
	for _, arg := range cmd.Argv {
		if strings.Contains(arg, bashast.CommandSubstitutionPlaceholder) {
			return blockReview("Command contains $() command substitution")
		}
	...
	for _, envVar := range cmd.EnvVars {
		if strings.Contains(envVar.Value, bashast.CommandSubstitutionPlaceholder) {
			return blockReview("Command contains $() command substitution")
		}
	}
...
func reviewASTRedirect(command string, redir bashast.Redirect) ReviewResult {
	...
	if strings.Contains(target, bashast.CommandSubstitutionPlaceholder) {
		return blockReview("Command contains $() command substitution")
	}
