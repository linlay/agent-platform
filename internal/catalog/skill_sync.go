...
// manifestFile records which skills under {agentDir}/skills/ were copied
// from skills-market by reconcileDeclaredSkills. Manually-added skills (not in
// the manifest) are preserved on subsequent syncs.
const manifestFile = ".market-synced-skills"

// reconcileDeclaredSkills syncs the declared skills for an agent from the
// skills-market into {agentDir}/skills/{skillId}/ — matching Java's
// AgentSkillSyncService.
//
// Behavior:
//   - For each declared skillId, copy {marketDir}/{skillId}/ recursively to
//     {agentDir}/skills/{skillId}/, overwriting any previous copy.
//   - Remove previously-synced skills (per manifest) that are no longer
//     declared. Skills not in the manifest (manually placed) are preserved.
//   - Write the new manifest.
//
// Path traversal is rejected at every step.
func reconcileDeclaredSkills(agentDir string, declaredSkills []string, marketDir string) error {
	...
	// Remove previously-synced skills no longer declared.
...
func writeManifest(skillsDir string, ids []string) error {
	if len(ids) == 0 {
		// Best-effort cleanup if there's no longer anything to track.
