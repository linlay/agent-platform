...
func tokenCLIFixture(t *testing.T, withStatus bool) (*Manager, connector.Package) {
	t.Helper()
	sources := connector.Sources{ExternalRoot: t.TempDir(), StateRoot: filepath.Join(t.TempDir(), "state"), Owner: "user:alice"}
	cli := simpleCLI()
...
func TestTokenCLIValidatesCandidateEnvironmentBeforeCommit(t *testing.T) {
	...
	if _, err = os.Stat(filepath.Join(private, "home", "probe")); !os.IsNotExist(err) {
		t.Fatal("candidate status used real user HOME", err)
	}
	users, err := os.ReadDir(filepath.Join(pkg.PersistentRoot(), "users"))
	if err != nil {
		t.Fatal(err)
	}
	if len(users) != 1 {
		t.Fatalf("temporary validation credentials were retained: %d roots", len(users))
	}
...
func TestDisconnectCancelsTokenValidationWithoutLateCommit(t *testing.T) {
	sources := connector.Sources{ExternalRoot: t.TempDir(), StateRoot: filepath.Join(t.TempDir(), "state"), Owner: "alice"}
	...
	m := New(t.Context(), sources, nil).WithCredentialValidator(func(ctx context.Context, pkg connector.Package, values map[string]string) error {
		if pkg.Owner == sources.Owner {
			t.Error("validator received live user")
		}
