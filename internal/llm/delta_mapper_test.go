...
func TestDeltaMapper_DebugLLMChat(t *testing.T) {
	...
	inputs := mapper.Map(contracts.DeltaDebugLLMChat{
		TaskID:                          "task_1",
		ChatID:                          "chat_1",
		ProviderKey:                     "mock",
	...
	call, ok := inputs[0].(stream.InputDebugLLMChat)
	if !ok {
		t.Fatalf("expected InputDebugLLMChat, got %#v", inputs[0])
	}
	if call.TaskID != "task_1" || call.TraceFile != "chat_1/.llm-records/run_1_001.json" || call.RunSeq != 1 || call.Status != "ok" {
