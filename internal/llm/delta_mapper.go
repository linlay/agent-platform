...
func (m *DeltaMapper) Map(delta AgentDelta) []stream.StreamInput {
	...
	case DeltaContextCompact:
		...
		return []stream.StreamInput{stream.InputContextCompact{
			...
			RemainingRatio:             value.RemainingRatio,
			ReleasedRatio:              value.ReleasedRatio,
			TokensFreed:                value.TokensFreed,
			ToolsCleared:               value.ToolsCleared,
			ReasoningCleared:           value.ReasoningCleared,
			CompactCoveredMessages:     value.CompactCoveredMessages,
			L1KeepRecent:               value.L1KeepRecent,
			L1PreserveReasoning:        value.L1PreserveReasoning,
			ToolsKept:                  value.ToolsKept,
