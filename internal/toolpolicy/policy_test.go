...
func TestAllowsReadOnlyUsesBuiltinAllowlistAndExplicitMetadata(t *testing.T) {
	tests := []struct {
		...
		{name: "agent run status is read only", def: api.ToolDetailResponse{Name: "agent_run_status", Meta: map[string]any{"kind": "backend", "sourceCategory": "platform", "readOnly": true}}, found: true, want: true},
		{name: "agent run query remains denied", def: api.ToolDetailResponse{Name: "agent_run_query", Meta: map[string]any{"kind": "backend", "sourceCategory": "platform", "readOnly": true}}, found: true, want: false},
		{name: "agent run interrupt remains denied", def: api.ToolDetailResponse{Name: "agent_run_interrupt", Meta: map[string]any{"kind": "backend", "sourceCategory": "platform", "readOnly": true}}, found: true, want: false},
