package tools

import "time"

var lunarInfo = []int{
	0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
	0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
	0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
	0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
	0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
	0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573, 0x052d0, 0x0a9a8, 0x0e950, 0x06aa0,
	0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
	0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b5a0, 0x195a6,
	0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
	0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60, 0x096d5, 0x092e0,
	0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
	0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
	0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
	0x05aa0, 0x076a3, 0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
	0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
	0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0,
	0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
	0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
	0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
	0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0, 0x0d150, 0x0f252,
	0x0d520,
}

var lunarMonthNames = []string{"正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊"}
var lunarDayPrefixes = []string{"初", "十", "廿", "卅"}
var lunarDayNumbers = []string{"零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"}
var heavenlyStems = []string{"甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"}
var earthlyBranches = []string{"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"}

var baseSolarDate = time.Date(1900, 1, 31, 0, 0, 0, 0, time.UTC)

type lunarDate struct {
	year      int
	month     int
	day       int
	leapMonth bool
}

func lunarDateText(dateTime time.Time) string {
	date := time.Date(dateTime.Year(), dateTime.Month(), dateTime.Day(), 0, 0, 0, 0, time.UTC)
	value, ok := solarToLunar(date)
	if !ok {
		return "超出支持范围"
	}
	return ganzhiYear(value.year) + "年" + leapPrefix(value.leapMonth) + lunarMonthName(value.month) + "月" + lunarDayName(value.day)
}

func solarToLunar(solarDate time.Time) (lunarDate, bool) {
	if solarDate.Before(baseSolarDate) {
		return lunarDate{}, false
	}
	offset := int(solarDate.Sub(baseSolarDate).Hours() / 24)
	lunarYear := 1900
	for lunarYear <= 2100 {
		days := lunarYearDays(lunarYear)
		if offset < days {
			break
		}
		offset -= days
		lunarYear++
	}
	if lunarYear > 2100 {
		return lunarDate{}, false
	}

	leapMonth := lunarLeapMonth(lunarYear)
	lunarMonth := 1
	isLeapMonth := false
	for lunarMonth <= 12 {
		daysInMonth := lunarMonthDays(lunarYear, lunarMonth)
		if isLeapMonth {
			daysInMonth = lunarLeapDays(lunarYear)
		}
		if offset < daysInMonth {
			break
		}
		offset -= daysInMonth
		if leapMonth > 0 && lunarMonth == leapMonth && !isLeapMonth {
			isLeapMonth = true
		} else {
			if isLeapMonth {
				isLeapMonth = false
			}
			lunarMonth++
		}
	}
	if lunarMonth > 12 {
		return lunarDate{}, false
	}
	return lunarDate{
		year:      lunarYear,
		month:     lunarMonth,
		day:       offset + 1,
		leapMonth: isLeapMonth,
	}, true
}

func lunarYearDays(year int) int {
	sum := 348
	info := lunarInfoForYear(year)
	for bit := 0x8000; bit > 0x8; bit >>= 1 {
		if info&bit != 0 {
			sum++
		}
	}
	return sum + lunarLeapDays(year)
}

func lunarLeapDays(year int) int {
	if lunarLeapMonth(year) == 0 {
		return 0
	}
	if lunarInfoForYear(year)&0x10000 != 0 {
		return 30
	}
	return 29
}

func lunarLeapMonth(year int) int {
	return lunarInfoForYear(year) & 0xF
}

func lunarMonthDays(year int, month int) int {
	if lunarInfoForYear(year)&(0x10000>>month) != 0 {
		return 30
	}
	return 29
}

func lunarInfoForYear(year int) int {
	index := year - 1900
	if index < 0 || index >= len(lunarInfo) {
		return 0
	}
	return lunarInfo[index]
}

func lunarMonthName(month int) string {
	if month < 1 || month > 12 {
		return "未知"
	}
	return lunarMonthNames[month-1]
}

func lunarDayName(day int) string {
	if day <= 0 || day > 30 {
		return "未知"
	}
	if day == 10 {
		return "初十"
	}
	if day == 20 {
		return "二十"
	}
	if day == 30 {
		return "三十"
	}
	prefix := day / 10
	number := day % 10
	return lunarDayPrefixes[prefix] + lunarDayNumbers[number]
}

func ganzhiYear(year int) string {
	stemIndex := (year - 4) % 10
	if stemIndex < 0 {
		stemIndex += 10
	}
	branchIndex := (year - 4) % 12
	if branchIndex < 0 {
		branchIndex += 12
	}
	return heavenlyStems[stemIndex] + earthlyBranches[branchIndex]
}

func leapPrefix(leap bool) string {
	if leap {
		return "闰"
	}
	return ""
}
