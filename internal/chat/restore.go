...
func (s *FileStore) RestoreArchivedChat(archived ArchivedChat) (Summary, error) {
	...
	_, err = tx.Exec(`INSERT INTO CHATS (
			CHAT_ID_, CHAT_NAME_, OWNER_TYPE_, AGENT_KEY_, AGENT_MODE_, TEAM_ID_, SOURCE_, SOURCE_CHANNEL_, CREATED_AT_, UPDATED_AT_, LAST_RUN_AT_, LAST_RUN_ID_, LAST_RUN_CONTENT_, READ_RUN_ID_, READ_AT_,
		...
		chatID, archived.Summary.ChatName, normalizedStoredOwnerType(archived.Summary.OwnerType, archived.Summary.AgentKey, archived.Summary.TeamID), archived.Summary.AgentKey, normalizeStoredAgentMode(archived.Summary.AgentMode, archived.Summary.OwnerType), nilIfEmpty(archived.Summary.TeamID), archived.Summary.Source, archived.Summary.SourceChannel,
	...
	for _, run := range archived.Runs {
		_, err = tx.Exec(`INSERT INTO RUNS (
			...
			run.RunID, run.ChatID, normalizedStoredOwnerType(run.OwnerType, run.AgentKey, run.TeamID), run.AgentKey, normalizeStoredAgentMode(run.AgentMode, run.OwnerType), nilIfEmpty(run.TeamID), run.InitialMessage, run.AssistantText, run.FinishReason,
