...
func (s *FileStore) DeriveChat(request DeriveChatRequest) (DeriveChatResult, error) {
	...
	if isTeamOwner(targetAgentKey, targetSourceRun.TeamID) {
		targetAgentKey = ""
	}
	tx, err := s.db.Begin()
	if err != nil {
	...
	_, err = tx.Exec(`INSERT INTO CHATS (
			CHAT_ID_, CHAT_NAME_, AGENT_KEY_, AGENT_MODE_, TEAM_ID_, SOURCE_, CREATED_AT_, UPDATED_AT_, LAST_RUN_AT_, LAST_RUN_ID_, LAST_RUN_CONTENT_, READ_RUN_ID_, READ_AT_,
...
func derivedRunAgentKey(run RunSummary) string {
