...
func TestCandidateFromRunTraceExtractsWorkflowFields(t *testing.T) {
	trace := chat.RunTrace{
		RunID:         "run-1",
		AssistantText: "Before deploy, ensure the automation config is reviewed.\nStep 1: update the automation file.\nStep 2: reload the service.\nIf reload fails, rollback immediately.\nSuccess means the job is triggered and health checks pass.",
		Query: &chat.QueryLine{
			Query: map[string]any{"message": "部署并验证 automation 配置变更"},
		},
	}
	...
	if candidate.Intent != "部署并验证 automation 配置变更" {
		t.Fatalf("unexpected intent: %#v", candidate.Intent)
	}
	if len(candidate.Preconditions) == 0 || candidate.Preconditions[0] != "Before deploy, ensure the automation config is reviewed" {
		t.Fatalf("unexpected preconditions: %#v", candidate.Preconditions)
	}
	if len(candidate.Steps) < 2 || candidate.Steps[0] != "Step 1: update the automation file" || candidate.Steps[1] != "Step 2: reload the service" {
