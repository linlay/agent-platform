...
3. Read skill documentation first: When a skill applies, copy its exact instructionsPath from the catalog into file_read.file_path before acting or running its CLI. Preserve @skills, @skills-center, or @connectors exactly as listed; never construct the path from skillId. If reading fails, check the catalog path, not other directories or same-named copies.
