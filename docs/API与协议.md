...
| POST | `/api/admin/skill-packages/import` | query: `key`、`version`、可选 `adopt`；raw ZIP body | 原子校验并安装或更新技能包，返回包状态与实际安装的子技能 |
...
`GET /api/admin/skill-packages` 是 `.package` 状态的只读投影，供 Desktop 将包内子技能识别为已安装并使用 manifest 版本比较；它不返回本地路径或 ZIP。`POST /api/admin/skill-packages/import` 是 Desktop Market 的技能包交付入口，不接受文件系统路径或 multipart。调用方以 `application/zip` 原样传入动态包，Platform 将请求体写入进程临时文件，完成 ZIP 安全检查、`manifest.json` 包身份与版本校验、必选子技能存在性以及每个子技能的标准 Skill 校验后，才在同一事务中替换 `skills-center/<skill-id>/`。同名独立 Skill 与包内内容一致时自动纳入包管理，保留原目录；逐项比较所有文件字节、目录结构与文件执行位，拒绝软链和特殊文件，仅当包中没有根 `skill.json` 且本地该文件严格匹配 Desktop 安装元数据结构（id/name/version/description/tags）时忽略该文件用于一致性判定。其他包已占用的 Key、安装目标名称大小写冲突仍返回 409，禁止接管。独立 Skill 内容不同返回 409 `skill_package_adoption_required`，`data.error.adoption` 包含 `archiveSha256` 和 `skills:[{id,revision,changedPaths}]`；用户确认替换后，以 query `adopt` 传 JSON `{archiveSha256,expectedRevisions:{<skill-id>:<revision>}}`，必须绑定同一个 ZIP 和未变化的本地内容，失败不发布任何成员。revision 包含用于比较时忽略的本地元数据，防止确认期间的编辑丢失。统一 multipart ZIP 导入入口也支持同一 query 参数。确认替换后将原目录持久保留在技能中心相邻的隐藏 `.skill-package-backup-*` 目录，成功响应附 `backupPath`；备份不进入技能扫描，不自动删除，可由用户恢复。相同内容自动纳入只更新包归属；之后由包统一更新和卸载该成员。包名称随 `.package` 记录保存并在列表与导入响应中返回，历史记录缺少名称时客户端以 ID 回退显示。更新会同时移除新版本不再包含的旧子技能；删除仍被 Agent 使用的子技能或通过普通 Skill 接口绕过包状态同样返回 409。包内子技能可以通过专用删除接口单独卸载，该操作会同步移除子技能目录和包记录中的成员；删除最后一个子技能时一并删除包状态。任一校验、文件切换或 Catalog 重载失败时，子技能目录和包状态全部恢复。普通安装成功后只保留子技能目录与 `skills-center/.package/<package-id>.json`，请求临时 ZIP、staging 和普通事务 backup 均删除；显式替换独立技能时保留上述恢复备份；`.package` 不进入 Skill Catalog。整包卸载端点使用同一事务删除该包记录与剩余子技能，Catalog 重载失败同样回滚。
