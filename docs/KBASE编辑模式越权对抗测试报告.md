...
- main/editing 两种 stage 都使用 KBASE Workspace 作为 Workspace，并提供相同的五个文件工具。
- KBASE Workspace、当前 Chat 目录、其他 chatId 和 external 都先使用同一 AccessPolicy。
- 非 editing KBASE Workspace 可读不可 mutation；write/edit 不产生无意义 HITL。
- `full_access`、auto approve、writeRoots、hostAccess 和 approval 都不能放宽非 editing KBASE Workspace mutation。
...
- Workspace mutation 不返回同步 `kbase-index` hook。
...
对 external 绝对路径、`../` 目标、其他 chatId 和 Workspace symlink 目标：
...
以上放宽只适用于 external/其他 chatId。对非 editing KBASE Workspace，测试覆盖相对路径、绝对路径、已有文件、新文件、`../` 与 Chat 指向 Workspace 的 symlink；即使设置 `full_access`、auto approve、writeRoots、hostAccess 或预注册 exact/rule approval，仍返回 `kbase_editing_mode_required`。若管理员将 Workspace 配为 readonly root，则 AccessPolicy Block 先返回 path-blocked。
...
KBASE Workspace 文件工具回归覆盖：

- `.md`、`.txt`、`.html`、`.json` 和自定义文本扩展名；
- GB18030 等通用支持编码的读取、编辑和原编码写回；
- Workspace 已有文件未读先写失败，读后写成功；
- Workspace 新文件父目录不存在时返回 `kbase_editing_parent_missing`；
- glob/grep 不再过滤为 `.md`；
...
以下用例覆盖 KBASE Workspace/Chats 分离：
...
验证文件系统根、相等、互为父子和 symlink 实际重叠均产生 `invalid_kbase_workspace_overlap`；只有目标 Agent 从运行时 catalog 隔离，其他 Agent 保持可用，依赖 Team 的重叠成员进入 `InvalidAgentKeys`。
