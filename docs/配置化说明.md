...
| `BUDGET_HITL_TIMEOUT_MS` | `0` | `Advanced / operator` | 全局 HITL/awaiting 等待超时；配置值 `0` 表示未配置，未配置时内置 fallback 为 `600000` |
...
注意：配置项中的 `BUDGET_HITL_*_TIMEOUT_MS=0` 表示未配置；事件协议中的 `awaiting.ask.timeout=0` 表示该等待项无限等待、不自动超时。
