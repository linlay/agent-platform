...
本机源码验证 support package 时使用 `release-local/` 作为 Desktop 服务包镜像，不使用 `runtime/`。运行 `make build-local` 后二进制位于 `release-local/backend/agent-platform`，插件放在 `release-local/plugins/`；`runtime/` 继续作为 agents、chats、skills-market、registries、memory 等运行数据目录。
