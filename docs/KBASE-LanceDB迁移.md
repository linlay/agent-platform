...
`control.db` 采用唯一的当前 SQLite schema 标识，不支持任何旧 control database 的补列或迁移。若 `control.db` 缺失但 storageDir 中存在 `kbase.db`、其 WAL/SHM 文件、根 `manifest.json` 或旧 generation，启动会失败；服务绝不自动删除或备份。升级时由运维手工清空整个 KBASE storageDir，随后 refresh 从 workspace 冷建新的 Lance generation。
