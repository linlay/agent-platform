...
仍有 native CODER planning 执行者的活动 Run 在生成计划和等待确认期间都接受 steer。正在进行的模型调用先正常结束；若生成候选计划期间有尚未消费的 steer，后端跳过旧计划的确认，收尾旧 `finalize_planning` tool result，在同一 Run 内按 FIFO 注入文字/冻结图片并进入下一版 planning。若旧确认已经发布，steer 会唤醒等待并输出 `awaiting.answer(status:"error", error.code:"planning_superseded")`；两种情况都会发布 `planning.superseded { planningId, planningFile, awaitingId, reason:"steer" }`，不伪造 `request.submit` 或用户 reject。新计划需要重新确认；用户要求停止时模型可以直接结束规划。拒绝后的反馈阶段同样支持 steer。

steer 准入与 planning submit 使用同一临界区：steer 先被接受，旧 awaiting 立即变为已解决，迟到的 approve 返回 `409 already_resolved`；approve 先被接受并确定新 execution Run，旧 Run 的后续 steer 返回 `accepted:false,status:"unmatched"`，客户端应使用新的 execution RunID。确认框发布与 steer 同时发生时，旧框可能短暂出现，但随后必须由同一 awaiting 的失效 answer 关闭，不能启动执行。该机制不把普通 question/approval/form 改成可被 steer 自动回答，也不新增跨进程 suspended 等待项的 steer 恢复路径；后者仍通过 submit 恢复。
